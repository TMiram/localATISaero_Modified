...
cpu_usage= 0.0 #total gpu used in perentage
...
if os.path.exists("mem_temp.txt") : #remove older files if there are any
	os.remove("mem_temp.txt")
if os.path.exists("cpu_temp.txt") :
	os.remove("cpu_temp.txt")


mem_get_command = """ top -bn 1 |grep \"^ \" |awk -F\" \"  \'{print($10);}\'>> mem_temp.txt """  # -bn 1 force the mesure to be made only once. And we send this value into a temporary file
cpu_get_command = """ top -bn 1 |grep \"^ \" |awk -F\" \"  \'{print($10);}\'>> cpu_temp.txt"""
